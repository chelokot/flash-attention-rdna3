...
# --- Variable-length (packed / cu_seqlens) path -----------------------------
#
# Sequences are packed end-to-end into (total_tokens, heads, head_dim) with no
# padding; cu_seqlens[i]:cu_seqlens[i+1] is sequence i. The grid is sized to the
# longest sequence and each program early-exits on blocks past its own sequence.
# The heavy inner loops are the same device functions as the batched path; only
# the base-pointer setup (token offset instead of batch stride) and the flat
# per-token LSE / output layout differ.
